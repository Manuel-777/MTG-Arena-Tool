...
class FilterPanel {
  ...
  private filterCallback = (propertyString: string) => (filter: string | boolean | ColorFilter) => {
          ...
          (<div className={"columnB"}>
            {(this.tags.length || this.decks.length) && (
              <div className={"deckFiltersCont"}>
                ...
                {this.decks.length ? (
                  <WrappedReactSelect className={this.prefixId + "_query_deck spacer180"} options={this.decks.map((deck: { id: string })=> deck.id)} current={this.filters.deckId} callback={this.filterCallback('deckId')} optionFormatter={getDeckComponentForwarded(this.decks)} />
          ...
          <div className={"columnC"}>
            ...
            {this.showOppManaFilter ? (
              ...
              <div className={"sortDiv"}>
                ...
                <WrappedReactSelect className={this.prefixId + "_query_sort sortSelect filter_panel_select_margin"} options={sortSelect} current={this.filters.sort} callback={this.filterCallback("sort")} />
                <div className={"select_container filter_panel_select_margin"} />
              </div>
            ) : <div className={"spacer180"} />}
          </div>
        )}
